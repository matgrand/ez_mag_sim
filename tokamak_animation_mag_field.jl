...
# create tokamak wires
Rₜ = 3.0 # tokamak major radius
rₜ = 1.4 # tokamak minor radius
ρₛ = 100.0 # segment density in teh wires, [#segments]/[m]

# Toroidal Field Coils
Ntf = 10 # number of coils
δtf = 0.5
Htf = [(rₜ+δtf)*sin(t) for t in range(0, stop=2π, length=Ntf+1)][1:end-1] # toroidal field coil heights
Rtf = [Rₜ + (rₜ+δtf)*cos(t) for t in range(0, stop=2π, length=Ntf+1)][1:end-1] # toroidal field coil radii
Wtf = [[V3(Rtf[i]*cos(t), Rtf[i]*sin(t), h) for t in range(0,stop=2π,length=round(Int, 2π*Rtf[i]*ρₛ))] for (i, h) in enumerate(Htf)]

# Poloidal Field Coils
Npf = 10 # number of coils
δpf = 0.2
...
fig = Figure(size=(1700,1100), theme=theme_black())
