...
w3, I3 = create_wire([V3(-2, 1.3*sin(t), 1.6*cos(t)) for t in range(0, stop=2π, length=5)], 40.0, 1.77e-8, 1e-4, 0.01)
w4, I4 = create_wire([V3(1.8*sin(t), 2.0, 1.5*cos(t)) for t in range(0, stop=2π, length=9)], 40.0, 1.77e-8, 1e-4, 0.01)
...
const NP = 500 # number of particles
const NT = 1800 # number of time steps
...
linesegments!(ax, segments, color=colors, transparency=true)
for w in wires plot!(Point3f.(w), color=:red) end # plot the wires
